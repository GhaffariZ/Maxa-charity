...
> وضعیت قابلیت: 🟢 فعال و پیاده‌سازی‌شده بر اساس `page-list.php`, `template-create.php`, `component-create.php`, `page-view.php`
...
* 🟢 **منتشر شده (`published`):** صفحه بر روی وب‌سایت زنده است و عموم مراجعان می‌توانند آن را ببینند.
* 🟡 **پیش‌نویس (`draft`):** صفحه ذخیره شده اما فقط برای مدیران قابل مشاهده است و به عموم نشان داده نمی‌شود.
* 🟠 **در حال تعمیر (`suspended`):** دسترسی موقت به صفحه قطع شده و به مراجعان پیام در حال به‌روزرسانی نمایش داده می‌شود.
* 🔴 **حذف شده (`deleted`):** صفحه از لیست صفحات فعال خارج شده است.
