...
?>
<style>
.event-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:20px;flex-wrap:wrap}.event-toolbar h1{margin:0}.event-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(360px,1fr));gap:20px}.event-card{padding:0;overflow:hidden;position:relative;border-radius:18px}.event-card-cover{height:155px;background:linear-gradient(135deg,#0a6668,#003f4a);position:relative;overflow:hidden}.event-card-cover img{width:100%;height:100%;object-fit:cover;opacity:.85}.event-card-cover:after{content:'';position:absolute;inset:0;background:linear-gradient(0deg,rgba(0,35,40,.8),transparent 75%)}.event-card-cover .status{position:absolute;z-index:1;top:14px;right:14px}.event-card-body{padding:20px 22px}.event-card h2{font-size:17px;margin:0 0 8px;line-height:1.7}.event-meta{color:var(--color-muted);font-size:12px;display:flex;gap:14px;flex-wrap:wrap}.event-actions{display:flex;gap:8px;margin-top:18px;flex-wrap:wrap}.event-actions form{margin:0}.empty-state{padding:50px 20px;text-align:center;color:var(--color-muted)}.empty-state strong{display:block;color:var(--color-text);font-size:18px;margin-bottom:6px}@media(max-width:720px){.event-grid{grid-template-columns:1fr}}
</style>
...
<?php foreach ($events as $event): $status = ['draft'=>'پیش‌نویس','published'=>'منتشرشده','archived'=>'آرشیو'][$event['status']] ?? $event['status']; ?>
  <article class="card event-card">
    ...
    <div class="event-card-body"><h2><?= event_h($event['title']) ?></h2><div class="event-meta"><span>◷ <?= event_h(event_date_label($event['event_date'])) ?></span><span>خبر: <?= (int)$event['news_count'] ?></span><span>استاد: <?= (int)$event['speaker_count'] ?></span><?php if ((int)$event['banner_active']): ?><span class="badge hq">بنر فعال</span><?php endif; ?></div><div class="event-actions"><a class="tbtn" href="event-create.php?id=<?= (int)$event['id'] ?>">ویرایش</a><a class="tbtn" target="_blank" href="/event.php?slug=<?= rawurlencode($event['slug']) ?>">مشاهده صفحه</a><form method="post" action="event-delete.php" onsubmit="return confirm('این رویداد و محتوای وابسته حذف شود؟')"><input type="hidden" name="id" value="<?= (int)$event['id'] ?>"><?= csrf_field() ?><button class="tbtn danger" type="submit">حذف</button></form></div></div>
  </article>
